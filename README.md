...
EventEase is an ASP.NET Core MVC web application designed for managing venues, events, and bookings. It allows users to manage venue availability, log events, and create bookings with calculated costs and relational database constraints.

---

# Database Architecture & ERD Specifications

The application uses Microsoft SQL Server with Entity Framework Core and the following table structure:

- **`dbo_Venues`**: Stores venue details, capacity, hourly rate, amenities, status, and description.
- **`dbo_Events`**: Stores event title, client details, attendance, dates, status, and assigned venue ID.
- **`dbo_Bookings`**: Stores confirmed bookings connecting a venue and an event. Enforces a **1:1 relationship with Events** and a **1:N relationship with Venues**.

---

# Database Setup Script (`script.sql`)

The database script contains the following:
- **Table Creation**
- **Entity Integrity** (Primary Keys, Default Constraints, Check Constraints)
- **Referential Integrity** (Foreign Keys, Cascade/Restrict Rules, Unique Constraints, and Indexes)
